...
begin
	...
	using Distances
	using Distributions: Bernoulli, mean
	using LaTeXStrings
...
##### Define some functions that'll help us generate our sample data
"""
...
begin
	...
	function smooth_p(p, mod=0.1)
		# Adjust less the closer to the middle
		modd = abs(0.5 - p) * mod
		res = p > 0.5 ? p - modd : p + modd
		return res
	end
end
...
md"""
##### Generate a small and simple dataset of 192 10x10 binary images
"""
...
begin
	# imgs = [RGB.(stimuli[i]) for i in eachindex(stimuli)]
	# RGB.(flatstim)
	# RGB.(study)
	# RGB.(test)
end
...
md"""
##### Create a basic recognition memory task, where the first two-third images are the study set, and the final two-third are the test set (middle third are in both)
"""

# ╔═╡ 5ccfa15c-7360-41fd-a3e9-3ebd8b6d436d
study = flatstim[:,1:end-(NOBS÷3)];

# ╔═╡ beea5b82-3b9e-4c07-977d-e4cdb4e8e437
test  = flatstim[:,(NOBS÷3)+1:end];

# ╔═╡ 1a0c90c0-aedc-4a90-a541-e7fea0c7200a
md"""
##### Calculate the scaled hamming distance between each test stimulus and the closest study stimulus
"""

# ╔═╡ e165d569-6197-46c7-932c-2a922fe16451
dists = let
...
# Somewhat linear distribution of distances. Uncomment to see
# scatter(sort(dists))

# ╔═╡ d89494d2-f742-496a-a2ad-8c91298255e2
md"""
##### Parameters from real data that will be used for generating our simulated responses
"""
...
md"""
# Using the package
##### Now that we have our generated responses, we can use fit_logistic5 to those responses along with the scaled distance values. 
##### The resulting `MDIResult` object has the fitted parameters, the calculated `λ` and `Δ` scores, and some other intermediate values for further analysis
"""
...
md"""
##### Using the package is that simple. In a typical scenario, there would be more than one or 2 participant response vectors. In which case, calling fit_logistic5 can be called for each response and you have a vector of `MDIResults` ready for analysis!
# Plotting the results
##### Plot the final curve alongside the simulated responses and the original curve
"""

# ╔═╡ 046dea09-4d26-462e-b0fb-ca4e2bbbc94e
function plotcurves(dists, simresults, simMDI, expMDI;title="")
	# Unimportant plotting stuff
	rang = range(0,1,100)
	plt = plot(ylabel=L"P_{new}", xlabel="Distance", ylim=(-0.02,1.02))
	plt = scatter!(plt, dists, simresults; label="Participant Answers", title)
	plt = plot!(plt, rang, logistic5(rang, simMDI.params); label="Simulated: λ=$(round(simMDI.λ;digits=3)), Δ=$(round(simMDI.Δ;digits=3))")
	plt = plot!(plt, rang, logistic5(rang, expMDI.params); label="Expected: λ=$(round(expMDI.λ;digits=3)), Δ=$(round(expMDI.Δ;digits=3))")
...
MnemonicDiscriminationIndex = "4ce081f7-88d2-45e6-980f-ae8df33f05ea"
...
Images = "~0.26.1"
LaTeXStrings = "~1.3.1"
MnemonicDiscriminationIndex = "~0.4.4"
Plots = "~1.40.4"
StableRNGs = "~1.0.2"
"""
...
manifest_format = "2.0"
project_hash = "7a84be1987dd27110d6070482ece39a072e59ca1"
...
[[deps.MnemonicDiscriminationIndex]]
deps = ["Distributions", "LsqFit", "PrecompileTools", "QuadGK", "Random"]
git-tree-sha1 = "ad1c4aa7bbcee9f6c5a01ea36aada2226fc396f0"
uuid = "4ce081f7-88d2-45e6-980f-ae8df33f05ea"
version = "0.4.4"
